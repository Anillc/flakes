...
in {
    ...
    config = let
        ...
        bgpmap-interface =       optionalString (cfg.bgpmap-interface       != null) "--bgpmap-interface ${concatStringsSep "," cfg.bgpmap-interface}";
        ...
        navbar-all-servers-url = optionalString (cfg.navbar-all-servers-url != null) "--navbar-all-servers-url ${cfg.navbar-all-servers-url}";
    ...
    in mkIf cfg.enable {
        systemd.services.bird-lg-go-frontend = {
            ...
            script = "${cfg.package}/bin/frontend ${servers} ${domain} ${listen} ${proxy-port} ${whois} ${dns-interface} ${bgpmap-interface} ${title-brand} ${navbar-brand} ${navbar-brand-url} ${navbar-all-servers} ${navbar-all-servers-url} ${net-specific-mode} ${protocol-filter}";
        };
    };
}